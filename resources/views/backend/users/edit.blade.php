                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-name cairo" id="basic-layout-form">تعديل مستخدم</h4>
                                ...
                                <form class="form" action="{{ route('users.update', $user) }}" method="POST">
                                    ...
                                    <div class="form-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" disabled id="name" class="form-control" value="{{ old('name', $user->name) }}" placeholder="إسم المستخدم" name="name">
                                            @error('name')
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="email" disabled id="email" class="form-control" value="{{ old('email', $user->email) }}" placeholder="البريد الالكتروني" name="email">
                                            @error('email')
                                                <span class="text-danger">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="permissions[]" id="permissions" class="select2 form-control" multiple>
                                                @foreach ($permissions as $permission)
                                                    <option value="{{ $permission->id }}" {{ $user->permissions->find($permission->id) ? 'selected' : '' }}>{{ $permission->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('permissions')
                                                <span class="text-danger">{{ $message }}</span>
